...
DATA_DIRECTORY = './data/custom_real'
DATA_LIST_PATH = './dataset/custom_real_list/val.txt'
SAVE_PATH = './result/custom/'
...
def main():
    ...
    testloader = data.DataLoader(CustomRealDataSet(args.data_dir, args.data_list, crop_size=(512, 1024), resize_size=(1024, 512), mean=IMG_MEAN, scale=False, mirror=False),
                                    batch_size=batchsize, shuffle=False, pin_memory=True, num_workers=4)

    scale = 1.25
    testloader2 = data.DataLoader(CustomRealDataSet(args.data_dir, args.data_list, crop_size=(round(512*scale), round(1024*scale) ), resize_size=( round(1024*scale), round(512*scale)), mean=IMG_MEAN, scale=False, mirror=False),
                                    batch_size=batchsize, shuffle=False, pin_memory=True, num_workers=4)
    scale = 0.9
    testloader3 = data.DataLoader(CustomRealDataSet(args.data_dir, args.data_list, crop_size=(round(512*scale), round(1024*scale) ), resize_size=( round(1024*scale), round(512*scale)), mean=IMG_MEAN, scale=False, mirror=False),
                                    batch_size=batchsize, shuffle=False, pin_memory=True, num_workers=4)
...
if __name__ == '__main__':
    ...
    os.system('python compute_iou.py ./data/custom_real/label %s'%save_path)
