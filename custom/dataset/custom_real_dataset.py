...
class CustomRealDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, resize_size=(1024, 512), crop_size=(512, 1024), mean=(128, 128, 128), scale=False, mirror=True, ignore_label=255, autoaug = False):
        ...
        for name in self.img_ids:
            ...
            label_file = osp.join(self.root, "label/%s" % name.replace('jpg', 'png'))
            self.files.append({
                "img": img_file,
                "label": label_file,
                "name": name
...
if __name__ == '__main__':
    dst = CustomRealDataSet('../data/custom_real/', '../dataset/custom_real_list/train.txt', mean=(0,0,0), autoaug=True)
