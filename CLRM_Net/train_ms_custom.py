...
from dataset.custom_real_dataset import CustomRealDataSet
from dataset.custom_synthetic_dataset import CustomSyntheticDataSet
...
ITER_SIZE = 1
NUM_WORKERS = 8
DATA_DIRECTORY = './data/custom_synthetic'
DATA_LIST_PATH = './dataset/custom_synthetic_list/train.txt'
DROPRATE = 0.1
IGNORE_LABEL = 255
INPUT_SIZE = '440,224'
DATA_DIRECTORY_TARGET = './data/custom_real'
DATA_LIST_PATH_TARGET = './dataset/custom_real_list/val.txt'
INPUT_SIZE_TARGET = '440,224'
CROP_SIZE = '320, 160'
LEARNING_RATE = 2e-4
MOMENTUM = 0.9
MAX_VALUE = 7
...
SNAPSHOT_DIR = './snapshots/trail_1'
WEIGHT_DECAY = 0.0005
WARM_UP = 5000 # no warmup
LOG_DIR = './log'
...
def get_arguments():
    ...
    parser.add_argument("--only-hard-label",type=float, default=80,  
                         help="class balance.")
...
def main():
    ...
    trainloader = data.DataLoader(
        CustomSyntheticDataSet(args.data_dir, args.data_list, max_iters=args.num_steps * args.iter_size * args.batch_size,
    ...
    targetloader = data.DataLoader(CustomRealDataSet(args.data_dir_target, args.data_list_target,
    ...
    for i_iter in range(args.num_steps):
        ...
        if i_iter >= args.num_steps_stop - 1:
            print('save model ...')
            torch.save(Trainer.G.state_dict(), osp.join(args.snapshot_dir, 'Custom_' + str(args.num_steps_stop) + '.pth'))
            torch.save(Trainer.D1.state_dict(), osp.join(args.snapshot_dir, 'Custom_' + str(args.num_steps_stop) + '_D1.pth'))
            torch.save(Trainer.D2.state_dict(), osp.join(args.snapshot_dir, 'Custom_' + str(args.num_steps_stop) + '_D2.pth'))
            break

        if i_iter % args.save_pred_every == 0 and i_iter != 0:
            print('taking snapshot ...')
            torch.save(Trainer.G.state_dict(), osp.join(args.snapshot_dir, 'Custom_' + str(i_iter) + '.pth'))
            torch.save(Trainer.D1.state_dict(), osp.join(args.snapshot_dir, 'Custom_' + str(i_iter) + '_D1.pth'))
            torch.save(Trainer.D2.state_dict(), osp.join(args.snapshot_dir, 'Custom_' + str(i_iter) + '_D2.pth'))
